...
- **オーバーレイ可読性**: コマ内**ガター**・**強めの境界ガイド線**（外枠付き）・**各コマ左上に1〜8番号バッジ**（白角丸・黒枠・吹き出しより手前）。**四隅優先**で吹き出し配置、**2/8 はお客様側（右寄り）顔帯を広めに回避**（候補を順に試す）。台本はオーバーレイ側で**短文化**（**2行基本**・**4/8のみ最大3行**・2/8・5/8・6/8は文字数厳しめ）、句読点優先・省略記号なし、最小フォント13px前後。**番号バッジと上辺吹き出し**は上方向クリアランスで重なりにくくする。主線の **`engine.js` は未変更**。
...
- **レイアウト固定**: **exactly 8 panels / 2 columns × 4 rows**・均等コマ・太め黒枠ガターを必須とし、**3×3・4×2・ポスター・コラージュ・コマ内番号・入れ子コマ**を禁止する文言を入れる。**枠省略・装飾段組み禁止**・**厳密グリッド優先（雰囲気の8コマ風ではなく分割の正確さ）**（無字画像のコマ割りブレ対策。モデルは依然ブレうる）。
