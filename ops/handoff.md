...
- **短文余白 note（`buildNoteShortSpaced`）**の **H1直後**に **`buildNoteTitleCandidateBlock`** で **【タイトル案】** を3行（**言い切り**＝`coreMain` 優先、なければテーマ／**実話・違和感**／**気づき・本質**＝`coreConclusion` 優先、なければ題材別の一行）。題材は **`classifyNoteMidBranch`** と同系の **`classifyTitleTopic`**（`ops` は `default` に寄せる）で口コミ／価格／契約に軽く寄せる。重複時は代替一行へ。Kindle構造化 note 等には付けない（既定の短文 note のみ）
