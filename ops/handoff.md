...
- **1行メモ（超簡易）**: フォーム最上段の **1行メモ** にだけ入力しても生成可能。`｜` または `|` で **テーマ｜伝えたいこと｜結論** と分割（2分割はテーマ＋伝えたいこと）。**1行のみ**はその文字列を **タイトルテーマ** と **一番伝えたいこと** に共通で使う（各入力欄の表示は変えず、`getInputFromForm` の論理値のみ）。空なら従来の最小入力3欄どおり
