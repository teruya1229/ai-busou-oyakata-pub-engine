...
## 現在の状態（2026-03-29・10コマ主線・引継ぎ）

- **本日やったこと（記録・この節の追記）**: 10コマ主線への正式切替、フォーム補助文・例文・placeholder 混入対策（コミット `488c3e5`）、`templates.js` の旧8コマ文言ノイズ除去（`ddffa14`）、10コマ吹き出し短文化・終盤役割整理（`c411bae`）。主線の8→10切替は `5a3ff24`。
- **現在の状態**: 主線は **10コマ（1/10〜10/10）・2列×5行**、**1コマ生成 → 2×5合成 → 吹き出し重ね**。**legacy8**（`comicLegacy8`・8コマ吹き出し等）は後方互換で保持。10コマ経路は Node / vm で整合確認済み。**ブラウザ実機確認は未完了**。
- **品質**: テキスト主線はかなり整っている。次の本丸は**実ブラウザでの見え方**（特に **4/10・7/10・8/10・9/10・10/10**）。4/10・7/10 に説明感が残る可能性あり。
- **次回やること**: **Claude in Chrome**（または `http://127.0.0.1:8787/` 同一オリジン）で**通常入力**と **placeholder** を確認。**4/10・7/10・8/10・9/10・10/10** の読みやすさ・役割差。必要なら最小修正。
...
## 現在の状態（2026-03-29・1コマ生成→2×5合成・10コマ主線）
- **主線（画像）**：`buildAllOutputs` の **`comicSinglePanelPrompts10`**（1/10〜10/10を1コマ1枚用）→ UI の **10欄に URL 貼付** → **2×5に合成**（`comic-10panel-composed.png`）→ **吹き出し付き完成漫画**（`comicBubbleScript10` / `comicBubblePlacement10`・合成キャンバス優先）。**legacy8** 用の **8欄・2×4**・`comicSinglePanelPrompts8` 等は後方互換として残置。
- **単一コマプロンプト**（`buildPanelPrompt`）：各コマに **役割固定（絵だけで分かること）** を付与。導入・事件・行動ルール・読者への問いを絵で区別し、中盤は情報過多を抑える（**`comicPrompt` と共通**）。
- **補助**：従来の **統合画像プロンプト**（`comicUnifiedPrompt`）は残置

## 現在の状態（2026-03-29・吹き出し付き完成漫画プレビュー）
- 無字合成画像のプレビュー表示後、**吹き出し付き完成漫画プレビューを生成**で **10コマ**台本・配置に基づく Canvas オーバーレイ（`comic-10panel-with-bubbles.png` で保存可・**2×5**想定）。**legacy8** 用 UI（8コマ・2×4）も併存。
- **オーバーレイ**: ガター／境界ガイド／コマ番号バッジ／四隅優先の吹き出し／台本の短文化・最小フォント（`js/app.js`）。
- **統合画像プロンプト**（`buildUnifiedComicImagePrompt`）: **10コマ・2列×5行**を主線に合わせた指示（補助導線。詳細は `js/engine.js`）
...
## 今日やったこと（2026-03-28：customer_side 7/8 行動ルール）

### 内容

- **`js/engine.js`**: **`buildNoteConclusionNextLine`**（**customer_side**）を **`次は、話し始める前に一言だけ聞く。そう決めた。`** に微調整
- **`ops/handoff.md` / `ops/status.md`**: 本節を最小追記
- **確認**: 題材A・題材D・**Node `buildAllOutputs`**
...
## 今日やったこと（2026-03-28：題材 G/H/F の 1/8・3/8・6/8）
