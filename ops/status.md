...
## 現在の状態（2026-03-29・1コマ生成→2×4合成・主線）
- **主線（画像）**：`buildAllOutputs` の **`comicSinglePanelPrompts8`**（1/8〜8/8を1コマ1枚用にラップ）→ UI の **8欄に URL 貼付** → **2×4に合成**（`comic-8panel-composed.png`）→ **吹き出し付き完成漫画**（合成キャンバス優先。無ければ従来の無字1枚）。オーバーレイのコマ分割は **2列×4行** に統一（従来の4列×2行前提を廃止）
- **単一コマプロンプト**（`buildPanelPrompt`）：各コマに **役割固定（絵だけで分かること）** を付与。導入・事件・行動ルール・読者への問いを絵で区別し、3〜6は情報過多を抑える（**`comicPrompt` と共通**）。
- **補助**：従来の **統合画像プロンプト**（`comicUnifiedPrompt`）は残置

## 現在の状態（2026-03-29・吹き出し付き完成漫画プレビュー）
- 無字8コマ画像のプレビュー表示後、**吹き出し付き完成漫画プレビューを生成**で台本・配置に基づく Canvas オーバーレイ（`comic-8panel-with-bubbles.png` で保存可・2×4想定）
- **オーバーレイ**: ガター／**強めの境界ガイド**／**コマ番号1〜8バッジ（最前面）**／**四隅優先の吹き出し**（2/8はお客様顔寄り帯を広めに回避）／**台本の強い短文化（2行基本）**・最小フォント13px前後（`js/app.js` のみ）
- **統合画像プロンプト**（`buildUnifiedComicImagePrompt`）: **2列×4行・8コマ均等・黒枠**を必須、**3×3等の別グリッド・ポスター・コマ内番号**を禁止。**厳密グリッド優先**の一文あり（`js/engine.js`）
