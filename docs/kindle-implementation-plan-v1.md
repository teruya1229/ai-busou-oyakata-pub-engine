...
- **前提（現行MVP）**: 主線は **漫画原稿・8コマ・note**。**Kindle は将来の別モード**。以下の「3出力」は **旧設計の呼び方**で、実体は **buildAllOutputs の主線キー**に読み替える。
...
- 中間構造から既存の **原稿・8コマネーム・note**（必要に応じ旧短縮・X）を再利用生成する。
- 生成済み素材を Kindle節へ変換し、4節単位で Kindle章素材へ束ねる（**MVPの「構成を生成」で毎回Kindleを出す前提ではない**）。
...
```js
{
  episodeId: string,
  sectionTitle: string,
  introHook: string,        // 8コマネーム（旧短縮は任意）由来
...
- `engine.js`:
  - 既存の **原稿・8コマ・note** 生成は維持（X・Kindleは主線外／別モード）。
