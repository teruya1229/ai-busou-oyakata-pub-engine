...
- **4コマ漫画構成**は **出来事→引っかかり→気づき→次の一手**の骨格で、各コマは **今回の入力**（現場メモ・一番伝えたいこと・結論／学び）から組み立てます。**題材軸**（`js/engine.js` の `inferTopicAxis`／`topicAxis`）で中盤の言い回しを分け、**「認識のズレ」「共通認識」系**は入力にその要素があるときだけ条件付きで出します。コパイロットは登場人物としてのセリフに留め、論点の中心から外しています。
