...
- **用途別コピー**（入力欄の直下）: **noteタイトル案 / 本文のみ（投稿用）/ note本文（H1付き）/ 4コマ構成 / 4コマ統合プロンプト / Kindle節素材**を個別にコピー。**スタイル向けにまとめてコピー**は、先頭に **タイトル案**を付けたうえで、出力スタイルに応じて **note向け＝本文のみ＋4コマ**、**4コマ向け＝構成＋統合**（従来どおり note 本文ブロックは含めない）、**Kindle向け＝本文のみ＋節素材**、**未指定＝本文のみ＋4コマ＋統合**を `---` 区切りで連結します。
