...
- **note記事本文**は、見出し（`##`）ではなく **タイトル1行＋短文・空行**で並べるスタイルです（タイトルテーマに `AI武装親方｜` が重複して付かないよう吸収します）。**口コミ／レビュー系**の冒頭問いは、テーマ・芯に含まれる語（満足と口コミ／仲良く・お客／レビュー・評価など）に応じて言い回しが分かれます（`js/engine.js` の `noteOpeningQuestionLine`）。**結論（芯）が空**のときは、テーマ・学び・芯を束ねて**締めの段落**を口コミ系／それ以外・題材語で分岐します（`buildNoteFinalBlock`。結論ありのときは従来どおり結論のあとに固定寄りの締め）。
