...
5. **（主線）** **「1コマずつ画像生成用プロンプト」**を外部で8枚生成し、戻りの URL / data URL を **8コマの画像URL** 欄に貼り、**「2×4に合成」**で合成プレビューを作る。続けて **「吹き出し付き完成漫画プレビューを生成」**（合成があればそちらに重ね、なければ従来の無字1枚プレビューに重ねる）。必要なら **「完成漫画プレビュー画像を保存」**（`comic-8panel-with-bubbles.png`）。**（補助）** 従来どおり **統合プロンプト**で1枚生成してもよい。オーバーレイは **`js/app.js`** で **2列×4行**前提、**各コマ左上に1〜8番号バッジ**・境界ガイド・**四隅優先の吹き出し**（2/8はお客様顔帯を広めに回避）・台本短文化など（`engine.js` の原稿・台本生成は従来どおり）
