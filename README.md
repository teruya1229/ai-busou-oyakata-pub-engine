...
- **章/本確認用**で `===` で章を区切り、**本素材を確認**で **本の目次・企画たたき台**が出る。**本タイトル候補（メイン／キーワードから推したサブ）・本のコンセプト・目次（構成案）・読み順の意図・編集向けの締め**を先に出し、従来の **章一覧・各章要約**は末尾の互換ブロックに残す。溜めた note を一冊の Kindle にするときの **企画書・目次下書き**にそのまま貼りやすい形を優先する。
