...
- **4コマ漫画構成**は **出来事→引っかかり→気づき→次の一手**の骨格で、各コマは **今回の入力**（現場メモ・一番伝えたいこと・結論／学び）から組み立てます。**題材軸**（`js/engine.js` の `inferTopicAxis`／`topicAxis`）で中盤の言い回しを分け、**「認識のズレ」「共通認識」系**は入力にその要素があるときだけ条件付きで出します。コパイロットは登場人物としてのセリフに留め、論点の中心から外しています。**顧客感覚・接客・説明まわり**は **`customer_side`** として扱い、2コマ目は **お客様の反応**を優先し、**コパイロットのセリフは原則出さない**（必須表現があるときだけ例外）。**`inferTopicAxis`** は **価格・客層**を **営業（問い合わせ等）** より先に判定しつつ、**顧客目線・不親切・業界の常識と顧客の非常識**などは **`customer_side`** を **見積**より先に取りにいきます。**協力会社＋ミス／手順／マニュアル／再発**は **現場段取り**寄りに寄せます。**現場メモが空で芯だけある場合**は、`pickOneConcreteScene` で **題材軸ごとに1シーン**を内部生成してから **4コマ**と **note** を組み立てます（抽象テーマの言い換えだけにしない）。
