...
## プロジェクト概要
現場で起きた出来事を入力すると、まず **漫画化前提の投稿文（1本の原稿・8ブロック）** を出し、そこから **4コマ構成・画像プロンプト・note本文**へ一方向に展開する静的Webツールです。**4コマを独立に先に作るのではなく**、原稿の**切り出し／再配置**としてコマを置きます。**note**は原稿の貼り付けに加え、**短い前後付け**（導入・締め）を補助。**Kindle**は後段の再編集（**原稿 → コマ・画像 → note補助 → Kindle**）。

主出力（`buildAllOutputs`）:
- **漫画原稿**（`comicManuscriptPost`・【導入】〜【読者への問い】）、**漫画タイトル**（`comicTitle`）、**漫画構成**（`comic`）、**コマ別描画プロンプト**（`comicPrompt`）、**統合画像プロンプト**（`comicUnifiedPrompt`）
- **note導入・締めの短文**（原稿の先頭／末尾ブロック由来・`noteIntroAssist` / `noteClosingAssist`）、**章・本用1行メモ**（`comicEpisodeSummary`）

補助・後段:
- **note用 Markdown**（`note`＝H1＋原稿、`noteBodyOnly`＝原稿のみ）、**従来の連載向け長文**は `buildNote`（Kindleプレビュー等が内部利用）、X投稿、Kindleプレビュー系

## MVPの目的
- まず動く最小構成を優先する
- **主導線は「投稿文＝漫画原稿」**（画面上も原稿→コマの順）。長文の別稿（`buildNote`）の改善を最優先にはしない
- build不要の静的構成。`index.html` を直接開いてもよいが、**4コマ画像APIと同一オリジンで使う**なら下記「推奨起動」
- 将来拡張しやすいように `js/app.js` `js/engine.js` `js/templates.js` に責務分離する（**コマ数は4に固定しない思想**。現状は4コマテンプレを利用）
...
## 芯の固定（任意入力）
- **この話で一番伝えたいこと / 絶対に入れたい表現 / 絶対にズラしたくない結論**を入力すると、**漫画原稿・4コマ・統合画像プロンプト**に同じ芯が反映されます。**すべて空なら従来どおり**（未入力と同じ動き）です。
...
- **4コマ漫画構成**は **原稿（8ブロック）をパースしたあと**、**出来事→引っかかり→気づき→次の一手**の枠に**切り出して**置きます（要約ではなく再配置）。フォールバックや題材軸（`inferTopicAxis`／`topicAxis`）・`frictionFromIncident` は従来どおり補助に使います。**顧客感覚**は **`customer_side`** で2コマ目はお客様の反応を優先、**コパイロット無音**時は画像指示でも出さない、などのルールは維持します。
