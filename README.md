...
- **漫画原稿**（`comicManuscriptPost`）、**タイトル**（`comicTitle`）、**ネーム本文**（`comic`・8コマ）、**8コマ吹き出し台本**（`comicBubbleScript8`・原稿から派生した画像載せ用の短行）、**8コマ吹き出し配置データ**（`comicBubblePlacement8`・種別・推奨位置・サイズ・行数目安。無字画像への後載せ用）、**コマ別描画プロンプト**（`comicPrompt`）、**統合画像プロンプト**（`comicUnifiedPrompt`）。**UI**で無字プレビュー後に **吹き出し付き完成漫画プレビュー**（Canvas 重ね・`comic-8panel-with-bubbles.png` 保存可・2×4想定）
...
5. **「吹き出し付き完成漫画プレビューを生成」**で、画面に出ている **8コマ吹き出し台本**と**配置データ**に沿って白地・黒文字の吹き出しを重ねた完成漫画プレビューを表示し、必要なら **「完成漫画プレビュー画像を保存」**（`comic-8panel-with-bubbles.png`。Canvas の `toDataURL` は同一オリジン／`data:` 画像が確実）。オーバーレイは **`js/app.js` のみ**で、コマ内ガター・境界ガイド・顔中心帯の回避・台本短文化・最小フォント14px 前後など可読性向けに調整する（`engine.js` の主線テキストは変えない）
