...
- **note記事本文**は、見出し（`##`）ではなく **タイトル1行＋短文・空行**で並べるスタイルです（タイトルテーマに `AI武装親方｜` が重複して付かないよう吸収します）。**口コミ／レビュー系**の冒頭問いは、テーマ・芯に含まれる語（満足と口コミ／仲良く・お客／レビュー・評価など）に応じて言い回しが分かれます（`js/engine.js` の `noteOpeningQuestionLine`）。
