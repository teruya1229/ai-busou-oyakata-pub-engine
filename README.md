...
## プロジェクト概要
現場で起きた出来事を入力すると、**実話漫画（現状は4コマ資産）を主役**に、構成・コマ文・画像生成プロンプトを一括で出す静的Webツールです。**noteは補助**（短い導入・締め・必要なら長文本文）で、**Kindleは後段の再編集**を想定します（**漫画 → note補助 → Kindle再編集**）。

主出力（`buildAllOutputs`）:
- **漫画タイトル**（`comicTitle`）、**漫画構成**（`comic`）、**コマ別描画プロンプト**（`comicPrompt`）、**統合画像プロンプト**（`comicUnifiedPrompt`）
- **note導入・締めの短文**（`noteIntroAssist` / `noteClosingAssist`）、**章・本用1行メモ**（`comicEpisodeSummary`）

補助・後段:
- 長文 **note本文**（`note` / `noteBodyOnly`・詳細は従来どおり）、X投稿、Kindleプレビュー系

## MVPの目的
- まず動く最小構成を優先する
- **主導線は漫画**（画面上も先に表示）。長文note本文の改善を最優先にはしない
...
## 芯の固定（任意入力）
- **この話で一番伝えたいこと / 絶対に入れたい表現 / 絶対にズラしたくない結論**を入力すると、**漫画構成・統合画像プロンプト・note（長文・短文）**に同じ芯が反映されます。**すべて空なら従来どおり**（未入力と同じ動き）です。
- **現場で起きたこと / 今回の学び**は完成文ではなく**素材メモ**でよいです（短文・箇条書き風でも可）。note本文では芯・テーマに合わせて**自然な文章に再構成**されます。
- **note記事本文**は、見出し（`##`）ではなく **短文・空行**で並べるスタイルです。**`buildNote` の戻り**は先頭に **H1（`#` ＋タイトル）** を付けます。**本文のみ**（H1 なし・note の本文欄向け）は **`buildAllOutputs` の `noteBodyOnly`**（`stripNoteLeadingHeading` で先頭の `# …` 行と直後の空行を除去）を参照します（タイトルテーマに `AI武装親方｜` が重複して付かないよう吸収します）。**【タイトル案】**（言い切り／実話・違和感／気づき・本質の3方向）は本文とは別枠に出し、note のタイトル欄用にコピーできます（`buildNoteTitleCandidateBlock`／`noteTitleSuggestions`）。**口コミ／レビュー系**の冒頭問いは、テーマ・芯に含まれる語（満足と口コミ／仲良く・お客／レビュー・評価など）に応じて言い回しが分かれます（`js/engine.js` の `noteOpeningQuestionLine`）。**結論（芯）が空**のときは、テーマ・学び・芯を束ねて**締めの段落**を口コミ系／それ以外・題材語で分岐します（`buildNoteFinalBlock`。結論ありのときは従来どおり結論のあとに固定寄りの締め）。**実話のあと「反転→なぜ」**の一行は、口コミ／価格・客層／契約・相性／現場改善などに応じて分岐し、学び行と先頭がかぶりすぎるときは別の言い回しに寄せます（`buildNoteTurnAndWhy`）。
...
- **4コマ漫画構成**は **出来事→引っかかり→気づき→次の一手**の骨格で、各コマは **今回の入力**（現場メモ・一番伝えたいこと・結論／学び）から組み立てます。**題材軸**（`js/engine.js` の `inferTopicAxis`／`topicAxis`）で中盤の言い回しを分け、**「認識のズレ」「共通認識」系**は入力にその要素があるときだけ条件付きで出します。コパイロットは登場人物としてのセリフに留め、論点の中心から外しています。**顧客感覚・接客・説明まわり**は **`customer_side`** として扱い、2コマ目は **お客様の反応**を優先し、**コパイロットのセリフは原則出さない**（必須表現があるときだけ例外）。**4コマ統合画像プロンプト**（`buildUnifiedComicImagePrompt`）も **`customer_side`** で **親方＋お客様**を主役とし、**コパイロット無音の回は画像指示でも登場させない**（端役にする場合は小さく）。**`inferTopicAxis`** は **価格・客層**を **営業（問い合わせ等）** より先に判定しつつ、**顧客目線・不親切・業界の常識と顧客の非常識**などは **`customer_side`** を **見積**より先に取りにいきます。**協力会社＋ミス／手順／マニュアル／再発**は **現場段取り**寄りに寄せます。**現場メモが空で芯だけある場合**は、`pickOneConcreteScene` で **題材軸ごとに1シーン**を内部生成してから **4コマ**と **note** を組み立てます（抽象テーマの言い換えだけにしない）。
...
- **用途別コピー**（生成後）: **漫画タイトル・構成・統合プロンプト**などを個別にコピー。**スタイル向けにまとめてコピー**は、既定で **漫画タイトル→構成→プロンプト→note導入・締め→（任意で）長文本文**の順を優先します。**note を投稿する**ときは **note投稿用コピー**（折りたたみ内）または **導入・締め短文**＋漫画タイトルで足りることもあります。
...
- **投稿前の確認メモ**（`buildNotePrePublishCheck`）: **note記事本文**の直後に、本文のみ・タイトル案ブロックを材料にした**軽いチェック**（良い点／気になる点／投稿前に1つだけ見るなら）を表示します。採点ではなく目安で、コピー導線は追加しません。
