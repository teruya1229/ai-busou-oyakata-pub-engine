...
5. **「吹き出し付き完成漫画プレビューを生成」**で、画面に出ている **8コマ吹き出し台本**と**配置データ**に沿って白地・黒文字の吹き出しを重ねた完成漫画プレビューを表示し、必要なら **「完成漫画プレビュー画像を保存」**（`comic-8panel-with-bubbles.png`。Canvas の `toDataURL` は同一オリジン／`data:` 画像が確実）
