...
- **8コマ標準ネーム**は **原稿（8ブロック）をパースしたあと**、**1/8〜8/8 にそのまま載せます**（まずは圧縮しない）。**旧4コマ短縮**は `comicLegacy4` で、出来事→引っかかり→気づき→次の一手の枠に再配置した版です。プロンプト側の題材補助（`inferTopicAxis`／`topicAxis`）・`frictionFromIncident` は従来どおり補助に使います。**顧客感覚**は **`customer_side`** で2コマ目はお客様の反応を優先、**コパイロット無音**時は画像指示でも出さない、などのルールは維持します。
