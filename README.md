...
- **note記事本文**は、見出し（`##`）ではなく **タイトル1行＋短文・空行**で並べるスタイルです（タイトルテーマに `AI武装親方｜` が重複して付かないよう吸収します）。**【タイトル案】**（言い切り／実話・違和感／気づき・本質の3方向）は本文とは別枠に出し、note のタイトル欄用にコピーできます（`buildNoteTitleCandidateBlock`／`buildAllOutputs` の `noteTitleSuggestions`）。**口コミ／レビュー系**の冒頭問いは、テーマ・芯に含まれる語（満足と口コミ／仲良く・お客／レビュー・評価など）に応じて言い回しが分かれます（`js/engine.js` の `noteOpeningQuestionLine`）。**結論（芯）が空**のときは、テーマ・学び・芯を束ねて**締めの段落**を口コミ系／それ以外・題材語で分岐します（`buildNoteFinalBlock`。結論ありのときは従来どおり結論のあとに固定寄りの締め）。**実話のあと「反転→なぜ」**の一行は、口コミ／価格・客層／契約・相性／現場改善などに応じて分岐し、学び行と先頭がかぶりすぎるときは別の言い回しに寄せます（`buildNoteTurnAndWhy`）。
...
- **用途別コピー**（入力欄の直下）: **noteタイトル案 / note本文 / 4コマ構成 / 4コマ統合プロンプト / Kindle節素材**を個別にコピー。**スタイル向けにまとめてコピー**は、先頭に **タイトル案**を付けたうえで、出力スタイルに応じて **note向け＝本文＋4コマ**、**4コマ向け＝構成＋統合**、**Kindle向け＝本文＋節素材**、**未指定＝本文＋4コマ＋統合**を `---` 区切りで連結します。
